  ...
  chrome.storage?.local?.get("token", function (data) {
    if (!!data.token) {
      chrome.storage.local.get("user", function (user) {
        ...
        if (!!user.user) {
          fetch(
            `https://api.twitch.tv/helix/streams/followed?user_id=${user.user.id}`,
            {
              method: "GET",
              headers: {
              ...
              response.json().then((data) => {
                chrome.storage.local.get(
                  ...
                  (previousOnlines: {
                    ...
                    const onlineStreams = data.data as {
                      id: string;
                      user_name: string;
                      title: string;
                    }[];
                    chrome.action.setBadgeText({
                      text: onlineStreams.length + "",
                    });
                    if (!previousOnlines.onlineStreams) {
                      chrome.storage.local.set({ onlineStreams }, () => {});
                    } else {
                      const prev = previousOnlines.onlineStreams;
                      const filtered = onlineStreams.filter(
                        (stream) =>
                          prev.find(
                            (prevStream) => prevStream.id === stream.id
                          ) === undefined
                      );
